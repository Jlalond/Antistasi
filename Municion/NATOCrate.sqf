...
//Standard Equipment (good for 5 people)
			...
			_crate addWeaponCargoGlobal			[bluRifle 			select 4, 	2 	];
			_crate addMagazineCargoGlobal		[bluRifleAmmo 		select 2, 	2	];
			...
			_crate addWeaponCargoGlobal [bluAT select 1, 1];
			_crate addMagazinCargoGlobal [blu
...
if (_NATOSupp < 50) then {
	...
	_crate addItemCargoGlobal ["B_UavTerminal",1];
			...
			_crate addWeaponCargoGlobal 		[bluSmallWpn 		select 1,	5	];
			_crate addMagazineCargoGlobal 		[bluSmallAmmo 		select 1,	25	];
			_crate addWeaponCargoGlobal 		[bluSmallWpn 		select 1,	3	];
			_crate addMagazineCargoGlobal 		[bluSmallAmmo 		select 1,	10	];
			_crate addWeaponCargoGlobal 		[bluSmallWpn 		select 1,	5	];
			_crate addMagazineCargoGlobal 		[bluSmallAmmo 		select 1,	3	];
			_crate addWeaponCargoGlobal 		[bluAT		 		select 0,	5	];	//AT tube
			_crate addItemCargoGlobal 			[bluATMissile		select 0,	5	];
			_crate addItemCargoGlobal			["SmokeShellRed"			,	25	];
			_crate addItemCargoGlobal			["SmokeShellGreen"			,	25	];
...
if(_NATOSupp > 20) then {
	_crate addWeaponCargoGlobal [bluRifle select 3, random 5 ];
	_crate addMagazineCargoGlobal [bluRifleAmmo select 3, 5 ];
	_crate addMagazineCargoGlobal [bluRifleAmmo select 2, 5 ];
	_crate addWeaponCargoGlobal [bluRifle select 4, random 5 ];
	_crate addWeaponCargoGlobal [bluSNPR select 0, random 5 ];
	_crate addMagazineCargoGlobal [bluSNPRAmmo select 0, random 5];
	_crate addWeaponCargoGlobal [bluLMG Select 2, 
};

if(_NATOSupp > 50) then {
	_crate addWeaponCargoGlobal [bluSNPR select 1, random 5 ];
	_crate addMagazineCargoGlobal [bluSNPRAmmo select 1, random 5];
	_crate addWeaponCargoGlobal [bluAT select 1, random 5 ];
	_crate addWeaponCargoGlobal [bluAA select 0, random 5 ];
	_crate addMagazineCargoGlobal [bluAAMissile select 0, random 15 ];
	_crate addItemCargoGlobal [bluScopes select 1, random 10 ];
	_crate addItemCargoGlobal [bluHelmet select 2, random 5 ];
	_crate addWeaponCargoGlobal [bluGL select 0, random 5 ];
	_crate addWeaponCargoGlobal [bluLMG select 2, random 10 ];
	_crate addWeaponCargoGlobal [bluLMG select 0, random 10 ];
	_crate addWeaponCargoGlobal [bluSNPR select 2, 2 ];
	_crate addMagazineCargoGlobal [bluSNPRAmmo select 2, 2 ];
};
